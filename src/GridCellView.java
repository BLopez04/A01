    ...
    public GridCellView(int x, int y) {
        ...
        addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mousePressed(java.awt.event.MouseEvent e) {
                GridCell current = board.get(x, y);

                if (current == GridCell.START || current == GridCell.END) {
                    return;
                }
                ...
                if (clickCount == 1) {
                    board.set(x, y, GridCell.START);
                } else if (clickCount == 2) {
                    board.set(x, y, GridCell.END);
                } else {
                    board.set(x, y,
                            current == GridCell.OBSTACLE ? GridCell.EMPTY : GridCell.OBSTACLE);
                }
            }
        });
    }
